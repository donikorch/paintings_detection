...
class Image():
    def __init__(self, i, j, tp):
        self.tp = tp
        self.image = cv2.imread(f'paintings/artist{i + 1}/{j + 1}.jpg')
        self.image_gray = cv2.imread(f'paintings/artist{i + 1}/{j + 1}.jpg', 0)
    ...
    def get_image(self):
        if self.tp == 'normal':
            ...
            return cv2.resize(self.image, (600, 600))
        
        elif self.tp == 'gray_r':
            return cv2.resize(self.image_gray, (600, 600))
...
class Program():
    ...
    def get_artist(self, i):
        if i == 0:
            return 'Винсент Ван Гог'
        
        elif i == 1:
            return 'Леонардо да Винчи'
        
        elif i == 2:
            return 'Пабло Пикассо'
    ...
    def show_result(self):
        ...
        for i in range(5):
            for j in range(len(self.tests[0])):
                ...
                ax1.set_xticks([])
                ax1.set_yticks([])
                ax1.set_title('Автор: ' + str(self.get_artist(i)))
                
                artist_1 = self.get_artist(self.results[0][i][j][2]) 
                artist_2 = self.get_artist(self.results[1][i][j][2]) 
                artist_3 = self.get_artist(self.results[2][i][j][2]) 
                
                ax1.set_xlabel('HOG: ' + str(artist_1) + 
                               '\nHoughLines: ' + str(artist_2) + 
                               '\nGaborFilter: ' + str(artist_3))
